...
sudo docker run --name annovar_db_get --volumes-from data_volumes -i -t compbio/ngseasy-annovar:v0.9.1 /bin/bash /usr/local/pipeline/annovar/get_annovar_gene_databases.sh

sudo docker run --volumes-from data_volumes -i -t compbio/ngseasy-annovar:v0.9.1 /bin/bash /usr/local/pipeline/annovar/get_annovar_databases.sh
